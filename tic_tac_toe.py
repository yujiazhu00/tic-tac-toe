...
class Game:
    ...
    def computer_move(self):
        ...
        # check if the computer can win, and if it can, to play that move
        for i in range(0,3):
            for j in range(0,3):
                if self.board.is_empty_cell(i,j):
                    trial_move = Move(self.computer.counter,i,j)
                    self.board.add_move(trial_move)
                    if self.board.check_for_winner(self.computer):
                        self.board.remove_move(trial_move)
                        return Move(self.computer.counter, i, j)
                    elif not self.board.check_for_winner(self.computer):
                        self.board.remove_move(trial_move)
        #check if the human player is about to win, and if so play to stop him from winning
        for i in range(0,3):
            for j in range(0,3):
                if self.board.is_empty_cell(i,j):
                    trial_move_2 = Move(self.human.counter,i,j)
                    self.board.add_move(trial_move_2)
                    if self.board.check_for_winner(self.human):
                        self.board.remove_move(trial_move_2)
                        return Move(self.computer.counter, i, j)
                    elif not self.board.check_for_winner(self.human):
                        self.board.remove_move(trial_move_2)
        if self.board.is_empty_cell(0, 0):
            # Choose the top left
            return Move(self.computer.counter, 0, 0)
        if self.board.is_empty_cell(2, 2):
            # Choose the bottom right
            return Move(self.computer.counter, 2, 2)
        if self.board.is_empty_cell(0, 2):
            # Choose the top right
            return Move(self.computer.counter, 0, 2)
        if self.board.is_empty_cell(2, 0):
            return Move(self.computer.counter, 2, 0)
            # Choose the bottom left
        # so that the computer can see 2 steps ahead to win
        for i in range(0,3):
            for j in range(0,3):
                if self.board.is_empty_cell(i,j):
                    great_trial = Move(self.computer.counter,i,j)
                    self.board.add_move(great_trial)
                    for ni in range(0,3):
                        for nj in range(0,3):
                            if self.board.is_empty_cell(ni,nj):
                                great_trial_2 = Move(self.computer.counter,ni,nj)
                                self.board.add_move(great_trial_2)
                                if self.board.check_for_winner(self.computer):
                                    self.board.remove_move(great_trial)
                                    self.board.remove_move(great_trial_2)
                                    return Move(self.computer.counter,i,j)
                                elif not self.board.check_for_winner(self.computer):
                                    self.board.remove_move(great_trial)
                                    self.board.remove_move(great_trial_2)
        #so that the computer can anticipate the steps that the human can take to win 2 steps ahead
        for i in range(0,3):
            for j in range(0,3):
                if self.board.is_empty_cell(i,j):
                    hu_trial_move = Move(self.human.counter,i,j)
                    self.board.add_move(hu_trial_move)
                    for mi in range(0,3):
                        for mj in range(0,3):
                            if self.board.is_empty_cell(mi,mj):
                                hu_trial_move_2 = Move(self.human.counter,mi,mj)
                                self.board.add_move(hu_trial_move_2)
                                if self.board.check_for_winner(self.human):
                                    self.board.remove_move(hu_trial_move)
                                    self.board.remove_move(hu_trial_move_2)
                                    return Move(self.computer.counter,i,j)
                                elif not self.board.check_for_winner(self.human):
                                    self.board.remove_move(hu_trial_move)
                                    self.board.remove_move(hu_trial_move_2)
        if self.board.is_empty_cell(1, 1):
            # Choose the center
            return Move(self.computer.counter, 1, 1)
        else:
            return self.computer.randomly_select_cell()
    def play(self):
